    ...
    <div class="container">
        ...
        <h3>Add book </h3>
            <form action="{{route('books.update' , ['book' => $book])}}" method="post" enctype="multipart/form-data" class="col-md-6 offset-md-3">
                ...
                @method('put')
                <h4>Add book </h4>
                <input type="text" name="ISBN" value="{{ $book->ISBN }}" placeholder="book ISBN" maxlength="13" minlength="13" pattern="[0-9]{13}" class="form-control my-2" required>
                <div class="text-danger">
                    @error('ISBN')
                        {{ $message }}
                    @enderror
                </div>
                <input type="text" name="title" value="{{ $book->title }}" placeholder="book title" maxlength="255" class="form-control my-2">
                <div class="text-danger">
                    @error('title')
                        {{ $message }}
                    @enderror
                </div>
                <input type="number" name="price" value="{{ $book->price }}" placeholder="book price" step="0.01" min="0" max="99999999" class="form-control my-2">
                
                <input type="number" name="mortgage" value="{{ $book->mortgage }}" placeholder="book mortgage" step="0.01" min="0" max="99999999" class="form-control my-2">
